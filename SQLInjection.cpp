...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    ...
    std::smatch match;
    // ranges 0-1000, and all letters with combinations of capital letters small letters and numbers.
    std::regex ranges("([A-Za-z0-9]'='[A-Za-z0-9])|([0-9]|[1-9][0-9]|[1-9][0-9][0-9]|1000)");
    char* error_message;
    if (std::regex_search(sql, match, ranges))
    {
        std::cout << sql << "  detected by matching string\n";
        return false;
    }
